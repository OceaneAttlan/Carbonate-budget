...
# "Distinct carbonate budgets following tropicalisation of temperate reefs"
...
Figure3 <- ggplot(calcification_rate, aes(x = factor(Species, level=rev(c("Coelestra aspera", "Turbinaria sp", "Acropora sp", "Acropora yongei"))), 
  ...
  scale_color_manual(values = c("#B2DFDB", "#26A69A", "#004D40")) +
...
# Create Figure 2: Community % cover
mycolors4 <- c("hotpink", "cadetblue3", "coral4")
Figure2 <- ggplot(CoralComm_cover_orga, 
                   aes(x=Site,
                       y=Orga_sum_perc_cover, color=Organism, group= Organism)) + 
  facet_wrap(.~Scenario, scales="free",ncol=2)+
  geom_point(size = 5, alpha=0.2,position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0)) + 
  #geom_line(aes(group = Tag), alpha = 0.5, position = position_dodge(0.5)) +
  stat_summary(aes(group = Organism), fun = mean, geom = "point", size =10,position = position_dodge(0.75)) +
  stat_summary(aes(group = Organism),geom = "errorbar",  fun.args = list(mult = 1), #Standard errors
               width = 0.3,size=1.5, position = position_dodge(0.75)) +
  scale_color_manual(values = mycolors4)+
  labs(x=NULL, y = "Cover (%)")+
  theme(axis.text.x=element_text(angle=0,size=20,vjust=0.5,hjust=0.5,margin=margin(t=10)))+
  theme(axis.title.x=element_text(),
        axis.title.y=element_text(size=30,angle=90,margin=margin(r=20)),
        axis.text=element_text(size=20),
        axis.text.y=element_text(size=30,margin=margin(r=10)),
        strip.text.x = element_text(size = 30,margin=margin(b=10)),
        panel.background = element_rect(fill='transparent'),
